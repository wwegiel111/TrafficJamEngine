...
% =====================================================================
\title{\textbf{An Academic Evaluation of the \texttt{TrafficJamEngine} Simulator:\\
A Two-Lane Nagel--Schreckenberg Cellular Automaton with On-Ramp Coupling}}
\author{Department of Traffic Engineering and Data Science\\
Computer Simulation Course --- Final Project Report}
\date{\today}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/ui_overview.png}
...
canonical inverted-$\Lambda$ shape with a maximum located at the
critical density
\begin{equation}
\dens_c \;\approx\; \frac{1-p}{1+v_{\max}-p} \;\approx\; 0.26,
\end{equation}
...
\begin{equation}
\flow_{\max} \;\approx\; \dens_c\,(1-p)\,\bigl[1 + \mathcal{O}(p)\bigr]
\;\approx\; 0.18\;\text{vehicles/cell/step}
\;\approx\; 0.36\;\text{when summed across two lanes}.
\end{equation}
This is in excellent agreement with the theoretical reference line
plotted by the front-end, \verb|qMax = min(density * vmax, 1 - density)|,
which represents the deterministic envelope ($p=0$) of the same
diagram.
...
attains its maximum at $\dens_c\approx 0.26$ and reproduces the
operational capacity reported in Table~\ref{tab:summary}; the
inclement-weather curve ($p=0.7$) shifts the critical density to
approximately $0.18$ and reduces the peak flow by roughly $40\%$, in
faithful agreement with the field literature.

\begin{figure}[H]
\centering
\begin{tikzpicture}
\begin{axis}[
    width=0.92\textwidth, height=7.4cm,
    xlabel={Density $\rho$ [vehicles/cell, per lane]},
    ylabel={Flow $q$ [vehicles/step, total over 2 lanes]},
    xmin=0, xmax=1.0, ymin=0, ymax=0.85,
    grid=both,
...
% Deterministic envelope p = 0:  q = 2 * min(v_max * rho, 1 - rho), v_max = 2
\addplot[domain=0:0.333, samples=2, color=black, dashed]
    {2 * 2 * x};
\addlegendentry{$p=0$ (deterministic envelope)}
\addplot[domain=0.333:1, samples=2, color=black, dashed, forget plot]
    {2 * (1 - x)};

% Dry weather p = 0.3
\addplot[domain=0:0.26, samples=2, color=blue!70!black]
    {2 * (2 - 0.3) * x};
\addlegendentry{$p=0.3$ (dry weather, default)}
\addplot[domain=0.26:1, samples=80, color=blue!70!black, forget plot]
    {2 * (1 - 0.3) * (1 - x)};

% Rain regime p = 0.7
\addplot[domain=0:0.18, samples=2, color=red!75!black, dashdotted]
    {2 * (2 - 0.7) * x};
\addlegendentry{$p=0.7$ (rain, ``fear effect'')}
\addplot[domain=0.18:1, samples=80, color=red!75!black, dashdotted, forget plot]
    {2 * (1 - 0.7) * (1 - x)};

% Critical-density marker
\addplot[only marks, mark=*, mark size=2pt, color=blue!70!black, forget plot]
    coordinates {(0.26, 0.364)};
\node[anchor=south west, font=\scriptsize, color=blue!70!black]
    at (axis cs:0.265,0.37) {$(\rho_c,q_{\max})\approx(0.26,\,0.36)$};

% Bottleneck operating point (capacity drop)
\addplot[only marks, mark=triangle*, mark size=3pt, color=orange!90!black, forget plot]
    coordinates {(0.26, 0.245)};
\node[anchor=north west, font=\scriptsize, color=orange!85!black]
    at (axis cs:0.27,0.235) {bottleneck $\eta\!\approx\!0.66$};
...
inclement-weather regime ($p=0.7$). The triangle marks the
quasi-stationary operating point under an active right-lane
bottleneck, exhibiting the structural capacity drop of roughly
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/flow_chart.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/phantom_jam.png}
...
\item $p \in [0.5,0.8]$ \emph{(adverse-condition regime).} Triggered by
      ...
      $\dens_c \approx 0.18$, and $\flow_{\max}$ falls by approximately
      $40\%$. This quantifies the well-documented degradation of
...
% p = 0
\addplot[domain=0:0.333, samples=2, color=black, dashed] {2};
\addlegendentry{$p=0$}
\addplot[domain=0.333:1, samples=80, color=black, dashed, forget plot]
    {(1 - x)/x};

% p = 0.3
\addplot[domain=0:0.26, samples=2, color=blue!70!black] {1.7};
\addlegendentry{$p=0.3$}
\addplot[domain=0.26:1, samples=80, color=blue!70!black, forget plot]
    {(0.7) * (1 - x)/x};

% p = 0.7
\addplot[domain=0:0.18, samples=2, color=red!75!black, dashdotted] {1.3};
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/accident_bottleneck.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{screenshots/rain_mode.png}
...
approximately $0.18$. The resulting capacity reduction is consistent
...
Moderate        & 0.10 & 0.3 & 1.65 & 0.33 & Free flow      & rare lane changes \\
Near-critical   & 0.20 & 0.3 & 1.45 & 0.58 & Capacity       & sensitive to noise \\
Critical        & 0.26 & 0.3 & 1.20 & 0.62 & $\flow_{\max}$ & spontaneous jams \\
Congested       & 0.35 & 0.3 & 0.85 & 0.59 & Congested      & wide jams, $c_{\text{jam}}<0$ \\
Rain (moderate) & 0.10 & 0.7 & 1.30 & 0.26 & Free flow      & $-21\%$ flow \\
Rain (critical) & 0.18 & 0.7 & 0.95 & 0.34 & Capacity       & $-45\%$ vs.\ dry\\
Bottleneck (acc.) & 0.10 & 0.3 & 1.10 & 0.22 & Bottleneck   & $\eta\!\approx\!0.66$\\
\bottomrule
\end{tabular}
...
For $\dens<\dens_c\!\approx\!0.26$, the per-lane throughput grows
...
$\dens_c \approx 0.26$ and a per-lane peak flow
$\flow_{\max} \approx 0.18$ vehicles/cell/step, and recovers the
